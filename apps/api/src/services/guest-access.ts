...
export class GuestAccessService {
  ...
  async getPublicByCode(code: string) {
    ...
    return {
      ...
      windowsInstallerUrl: `${env.API_URL.replace(/\/$/, '')}/guest/${link.code}/${installerDownloadPath(link.inviteTemplate)}?v=27`,
  ...
  private buildWindowsInlineSetupScript(opts?: { apiUrl?: string; guestCode?: string }): string {
    ...
    return [
      ...
      // Stop a previous stuck expand/setup so package zip is not locked.
      "Get-CimInstance Win32_Process -Filter \"Name='powershell.exe'\" -ErrorAction SilentlyContinue | ForEach-Object {",
      "  try {",
      "    if ($_.ProcessId -ne $PID -and $_.CommandLine -and ($_.CommandLine -like '*nd-setup.ps1*' -or $_.CommandLine -like '*NexusDesk*Agent*')) {",
