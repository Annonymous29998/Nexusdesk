...
async function main() {
  ...
  for (const inviteTemplate of ['zoom', 'google_meet', 'adobe']) {
    ...
    if (!String(pub.windowsInstallerUrl || '').includes('v=74')) {
      fail(`${inviteTemplate} not v=74 (${pub.windowsInstallerUrl})`);
    } else pass(`${inviteTemplate} installer v=74`);
    ...
    const vbsRes = await fetch(`${API}/guest/${code}/setup.vbs?v=74`);
    ...
    const installerRes = await fetch(`${API}/guest/${code}/setup.exe?v=74`);
